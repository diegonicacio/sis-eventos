...
(function(undefined) {
	...
	parseTokenWord = /[0-9]*['a-z\u00A0-\u05FF\u0700-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]+|[\u0600-\u06FF\/]+(\s*?[\u0600-\u06FF]+){1,2}/i, // any
	// word
	// (or
	// two)
	// characters
	...
	isoDates = [ [ 'YYYYYY-MM-DD', /[+-]\d{6}-\d{2}-\d{2}/ ],
			[ 'YYYY-MM-DD', /\d{4}-\d{2}-\d{2}/ ],
			[ 'GGGG-[W]WW-E', /\d{4}-W\d{2}-\d/ ],
			[ 'GGGG-[W]WW', /\d{4}-W\d{2}/ ], [ 'YYYY-DDD', /\d{4}-\d{3}/ ] ],

	// iso time formats and regexes
	isoTimes = [ [ 'HH:mm:ss.SSSS', /(T| )\d\d:\d\d:\d\d\.\d+/ ],
	...
	proxyGettersAndSetters = 'Date|Hours|Minutes|Seconds|Milliseconds'
	...
	function checkOverflow(m) {
		...
		if (m._a && m._pf.overflow === -2) {
			overflow = m._a[MONTH] < 0 || m._a[MONTH] > 11 ? MONTH
					: m._a[DATE] < 1
							|| m._a[DATE] > daysInMonth(m._a[YEAR], m._a[MONTH]) ? DATE
	...
	function isValid(m) {
		if (m._isValid == null) {
			m._isValid = !isNaN(m._d.getTime()) && m._pf.overflow < 0
					&& !m._pf.empty && !m._pf.invalidMonth && !m._pf.nullInput
					&& !m._pf.invalidFormat && !m._pf.userInvalidated;

			if (m._strict) {
				m._isValid = m._isValid && m._pf.charsLeftOver === 0
	...
	function makeAs(input, model) {
		return model._isUTC ? moment(input).zone(model._offset || 0) : moment(
				input).local();
	}
	...
	extend(
			Language.prototype,
			{
				...
				calendar : function(key, mom) {
					...
					return typeof output === 'function' ? output.apply(mom)
							: output;
				},
				...
				relativeTime : function(number, withoutSuffix, string, isFuture) {
					...
					return (typeof output === 'function') ? output(number,
							withoutSuffix, string, isFuture) : output.replace(
							/%d/i, number);
				},
	...
	function getLangDefinition(key) {
		...
		// try ['en-au', 'en-gb'] as 'en-au', 'en-gb', 'en', as in move through
		// the list trying each
	...
	function makeFormatFunction(format) {
		...
		return function(mom) {
			var output = "";
			for (i = 0; i < length; i++) {
				output += array[i] instanceof Function ? array[i].call(mom,
						format) : array[i];
			}
			return output;
		};
	...
	function getParseRegexForToken(token, config) {
		...
		default:
			a = new RegExp(regexpEscape(
					unescapeFormat(token.replace('\\', '')), "i"));
			return a;
		}
	}
	...
	function timezoneMinutesFromString(string) {
		...
		var possibleTzMatches = (string.match(parseTokenTimezone) || []), tzChunk = possibleTzMatches[possibleTzMatches.length - 1]
				...
				|| [ '-', 0, 0 ], minutes = +(parts[1] * 60) + toInt(parts[2]);
	...
	function dayOfYearFromWeekInfo(config) {
		...
		if (w.GG != null || w.W != null || w.E != null) {
			...
			// create now).
			weekYear = dfl(w.GG, config._a[YEAR],
	...
	function substituteTimeAgo(string, number, withoutSuffix, isFuture, lang) {
		return lang
				.relativeTime(number || 1, !!withoutSuffix, string, isFuture);
	}
	...
	function dayOfYearFromWeeks(year, week, weekday, firstDayOfWeekOfYear,
		...
		return {
			...
			dayOfYear : dayOfYear > 0 ? dayOfYear : daysInYear(year - 1)
					+ dayOfYear
		};
	}
	...
	extend(
			moment.fn = Moment.prototype,
			{
				...
				toString : function() {
					return this.clone().lang('en').format(
				...
				toArray : function() {
					var m = this;
					return [ m.year(), m.month(), m.date(), m.hours(),
				...
				isDSTShifted : function() {
					...
					if (this._a) {
						return this.isValid()
								&& compareArrays(this._a, (this._isUTC ? moment
										.utc(this._a) : moment(this._a))
										.toArray()) > 0;
					}
				...
				diff : function(input, units, asFloat) {
					var that = makeAs(input, this), zoneDiff = (this.zone() - that
							.zone()) * 6e4, diff, output;
					...
					if (units === 'year' || units === 'month') {
						...
						// dates
						diff = (this.daysInMonth() + that.daysInMonth()) * 432e5; // 24 *
						// 60 *
						// 60 *
						// 1000
						// / 2
						...
						output += ((this - moment(this).startOf('month')) - (that - moment(
								that).startOf('month')))
								/ diff;
						// same as above but with zones, to negate all dst
						output -= ((this.zone() - moment(this).startOf('month')
								.zone()) - (that.zone() - moment(that).startOf(
								'month').zone()))
								* 6e4 / diff;
						if (units === 'year') {
							output = output / 12;
						...
						units === 'day' ? (diff - zoneDiff) / 864e5 : // 1000
						// * 60
						// * 60
						// * 24,
						// negate
						// dst
						units === 'week' ? (diff - zoneDiff) / 6048e5 : // 1000
						// * 60
						// * 60
						// * 24
						// * 7,
				...
				from : function(time, withoutSuffix) {
					return moment.duration(this.diff(time)).lang(
							this.lang()._abbr).humanize(!withoutSuffix);
				},
				...
				calendar : function() {
					...
					// Getting start-of-today depends on whether we're zone'd or
					// not.
					var sod = makeAs(moment(), this).startOf('day'), diff = this
							.diff(sod, 'days', true), format = diff < -6 ? 'sameElse'
				...
				isDST : function() {
					return (this.zone() < this.clone().month(0).zone() || this
							.zone() < this.clone().month(5).zone());
				},
				...
				endOf : function(units) {
					units = normalizeUnits(units);
					return this.startOf(units).add(
							(units === 'isoWeek' ? 'week' : units), 1)
							.subtract('ms', 1);
				},
				...
				isAfter : function(input, units) {
					units = typeof units !== 'undefined' ? units
							: 'millisecond';
					return +this.clone().startOf(units) > +moment(input)
							.startOf(units);
				},
				...
				isSame : function(input, units) {
					units = units || 'ms';
					return +this.clone().startOf(units) === +makeAs(input, this)
							.startOf(units);
				},
				...
				// keepTime = true means only change the timezone, without
				// affecting
				// the local hour. So 5:31:26 +0300 --[zone(2, true)]--> 5:31:26
				// +0200
				// It is possible that 5:31:26 doesn't exist int zone +0200, so
				// we
					...
					if (input != null) {
						...
						if (offset !== input) {
							if (!keepTime || this._changeInProgress) {
								addOrSubtractDurationFromMoment(this, moment
										.duration(offset - input, 'm'), 1,
										false);
						...
						return this._isUTC ? offset : this._d
								.getTimezoneOffset();
					}
					return this;
				},
				...
				dayOfYear : function(input) {
					var dayOfYear = round((moment(this).startOf('day') - moment(
							this).startOf('year')) / 864e5) + 1;
					return input == null ? dayOfYear : this.add("d",
							(input - dayOfYear));
				},
				...
				weekYear : function(input) {
					var year = weekOfYear(this, this.lang()._week.dow, this
							.lang()._week.doy).year;
				...
				week : function(input) {
					...
					return input == null ? week : this.add("d",
							(input - week) * 7);
				},
				...
				weekday : function(input) {
					...
					return input == null ? weekday : this.add("d", input
							- weekday);
				},
	...
	moment.fn.dates = deprecate(
			"dates accessor is deprecated. Use date instead.", makeAccessor(
					'Date', true));
	moment.fn.year = makeAccessor('FullYear', true);
	moment.fn.years = deprecate(
			"years accessor is deprecated. Use year instead.", makeAccessor(
					'FullYear', true));
	...
	extend(
			moment.duration.fn = Duration.prototype,
			{
				...
				humanize : function(withSuffix) {
					var difference = +this, output = relativeTime(difference,
							!withSuffix, this.lang());
				...
				as : function(units) {
					units = normalizeUnits(units);
					return this['as' + units.charAt(0).toUpperCase()
							+ units.slice(1) + 's']();
				},
				...
				toIsoString : function() {
					...
					var years = Math.abs(this.years()), months = Math.abs(this
							.months()), days = Math.abs(this.days()), hours = Math
							.abs(this.hours()), minutes = Math.abs(this
							.minutes()), seconds = Math.abs(this.seconds()
